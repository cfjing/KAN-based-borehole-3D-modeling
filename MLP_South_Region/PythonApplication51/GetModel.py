...
class CustumModule(torch.nn.Module):
    def __init__(self, in_pos_channels=CONFIG.NUM_FEATURE_POS, in_fac_channels=CONFIG.NUM_FEATURE_FAC, output_channels=CONFIG.NUM_CLASSES):
        ...
        self.pos_lin_1 = nn.Linear(in_pos_channels, CHANNEL * 1)
        self.pos_lin_2 = nn.Linear(CHANNEL * 1, CHANNEL * 2)
        self.pos_bn_1 = nn.LayerNorm(CHANNEL * 2)

        self.fac_lin_1 = nn.Linear(in_fac_channels, CHANNEL * 1)
        self.fac_lin_2 = nn.Linear(CHANNEL * 1, CHANNEL * 2)
        self.fac_bn_1 = nn.LayerNorm(CHANNEL * 2)

        self.out_lin_1 = nn.Linear(CHANNEL * 4, CHANNEL * 2)
        self.out_drop_1 = nn.Dropout(p=0.25)
        self.out_lin_2 = nn.Linear(CHANNEL * 2, CHANNEL * 1)
